...
def add_options():
	...
	parser.add_option('-a', '--user-agent', dest='agent', default=None, help='user agent you want to use in your requests.')
	parser.add_option('-t', '--tor', dest='tor', default=False, help='send requests anonymously.', action='store_true')
		...
		for user in users:
			for password in passwords:
				...
				if opts.agent is not None:
					...
					response = session.get(url, headers = {'User-agent': opts.agent})
				else:
